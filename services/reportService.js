...
export const buildSalesReport = async (fromDate, toDate) => {
  ...
  const waiterDocs = waiterIds.length
    ? await User.find({ _id: { $in: waiterIds } }).select("name role")
    : [];
...
export const createSalesReportWorkbook = async ({ report, orders, fromDate, toDate }) => {
  ...
  const paymentMethods = report.distribution.paymentMethods || {};
  const paymentTotal = Object.values(paymentMethods).reduce(
    (sum, amount) => sum + (Number(amount) || 0),
    0
  );
  const paymentEntries = Object.entries(paymentMethods).map(([method, amount]) => ({
